...
public class ExchangeCodec extends TelnetCodec {
    ...
    protected Object decode(Channel channel, InputStream inputStream, int readable, byte[] header) throws IOException {
        // check magic number.
        if (readable > 0 && header[0] != MAGIC_HIGH 
            ...
            if (header.length < readable) {
                ...
                inputStream.read(header, length, readable - length);
            }
            for (int i = 1; i < header.length - 1; i ++) {
                if (header[i] == MAGIC_HIGH && header[i + 1] == MAGIC_LOW) {
                    PositionableInputStream bis = ((PositionableInputStream) inputStream);
                    bis.position(bis.position() - header.length + i);
                    header = Bytes.copyOf(header, i);
                    break;
                }
            }
            return super.decode(channel, inputStream, readable, header);
        }
        // check length.
        ...
        try {
            return decodeBody(channel, inputStream, header);
        } finally {
            if (inputStream.available() > 0) {
                try {
                    if (logger.isWarnEnabled()) {
                        logger.warn("Skip input stream " + inputStream.available());
                    }
                    StreamUtils.skipUnusedStream(inputStream);
                } catch (IOException e) {
